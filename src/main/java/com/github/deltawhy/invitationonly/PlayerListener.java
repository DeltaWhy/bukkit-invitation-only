    ...
    public void onPlayerLogin(PlayerLoginEvent e) {
        ...
        if (plugin.isInvited(userid)) {
            if (plugin.getConfig().getBoolean("require-op-online", false) && !plugin.isOpOnline()) {
                ...
                if (plugin.getConfig().getBoolean("broadcast-failed-logins", true)) {
                    plugin.getServer().broadcastMessage(ChatColor.YELLOW + player.getName() + " tried to join but there is no OP online.");
                }
